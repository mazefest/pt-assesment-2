...
let empOne = new Employee('Colby', 'Thursday');
...
class Manager extends Employee {
    ...
    getEmployees() {
        ...
        this.employees.forEach((element, index) => {
            if (index === 0) {
                employeesString = employeesString + `${element}`;
            } else if (this.employees.length != 1 && index === this.employees.length - 1) {
                employeesString = employeesString + ` and ${element}`;
            } else {
                employeesString = employeesString + `, ${element}`;
            }
        });
...
let manager = new Manager('Terry', 'weekday mornings, weekday afternoons', ['Cece', 'Schmidt'])
