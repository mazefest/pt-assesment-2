...
const getUser = (userName) => { 
    return `Wlecome back ${userName}`;
}

console.log(`${getUser('mazefest')}`);
...
function canWeDeliver(zipCode) {
    return deliveryAreaZipCodes.includes(zipCode) ? 'Yes we can deliver in your zone' : 'Sorry, we can\'t deliver to that address'
}
...
function canWeDeliverTwo(zipCode) {
    let canWe = false;
    deliveryAreaZipCodes.forEach(deliverableZipCode => {
        if (zipCode === deliverableZipCode) {
            canWe = true;
        }
    })
    return canWe ? 'Yes we can deliver in your zone' : 'Sorry, we can\'t deliver to that address'
}
